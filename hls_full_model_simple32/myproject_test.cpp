...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      nnet::copy_data<float, input_t, 0, 32*32*1>(in, input_1);
      result_t layer10_out[1];

            // hls-fpga-machine-learning insert top-level-function
#ifdef  __BAMBU__
            m_param_alloc(0, sizeof(input_1));
            m_param_alloc(1, sizeof(layer10_out));
#endif
            myproject(input_1,layer10_out);
        ...
        for (unsigned i = 0; i < NUM_TEST_SAMPLES; i++) {
            ...
            input_t input_1[32*32*1];
            nnet::fill_zero<input_t, 32*32*1>(input_1);
            result_t layer10_out[1];
